# Sophia Hooks Reference

Use this reference when configuring `/data/.sophia/hooks.json`.
...
{
  "type": "command",
  "command": "python3 .sophia/hooks/check.py",
  "work_dir": "/data",
  "timeout": "10s",
  "on_error": "block"
}
...
- Environment includes `env` plus `SOPHIA_HOOK_EVENT`, `SOPHIA_HOOK_NAME`, `SOPHIA_BOT_ID`, and `SOPHIA_SESSION_ID`.
...
Calls a Sophia tool by name when a hook tool runner is available for that event.
...
```json
{
  ...
  "actions": [
    {
      "type": "command",
      "command": "mkdir -p .sophia && cat >> .sophia/workspace-commands.jsonl",
      "on_error": "ignore"
    }
  ]
}
