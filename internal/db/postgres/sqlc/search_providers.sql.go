...
DELETE FROM search_providers WHERE team_id = public.sophia_current_team_id() AND id = $1
`
...
SELECT id, name, provider, config, enable, created_at, updated_at, team_id FROM search_providers WHERE team_id = public.sophia_current_team_id() AND id = $1
`
...
SELECT id, name, provider, config, enable, created_at, updated_at, team_id FROM search_providers WHERE team_id = public.sophia_current_team_id() AND name = $1
`
...
SELECT id, name, provider, config, enable, created_at, updated_at, team_id FROM search_providers
WHERE team_id = public.sophia_current_team_id() AND provider = $1
ORDER BY created_at DESC
`
...
SET
  ...
  enable = $4,
  updated_at = now()
WHERE team_id = public.sophia_current_team_id() AND id = $5
RETURNING id, name, provider, config, enable, created_at, updated_at, team_id
`
