...
WHERE cv.team_id = public.sophia_current_team_id() AND s.team_id = public.sophia_current_team_id() AND cv.container_id = $1
  AND cv.version = $2
`
...
SELECT COALESCE(MAX(version), 0) + 1 FROM container_versions WHERE team_id = public.sophia_current_team_id() AND container_id = $1
`
