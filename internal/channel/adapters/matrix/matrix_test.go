...
func TestIsMatrixBotMentionedByMentionsMetadata(t *testing.T) {
	content := map[string]any{
		...
		"m.mentions": map[string]any{
			"user_ids": []any{"@sophia:example.com"},
		},
	}
	if !isMatrixBotMentioned("@sophia:example.com", content) {
...
func TestIsMatrixBotMentionedByFormattedBody(t *testing.T) {
	content := map[string]any{
		"body":           "hello Sophia",
		"formatted_body": `<a href="https://matrix.to/#/@sophia:example.com">Sophia</a> hello`,
	}
	if !isMatrixBotMentioned("@sophia:example.com", content) {
...
func TestMatrixBotMentionAliasesIncludeFullIDAndLocalpart(t *testing.T) {
	aliases := matrixBotMentionAliases("@sophia:example.com")
	if len(aliases) != 2 || aliases[0] != "@sophia:example.com" || aliases[1] != "@sophia" {
...
func TestIsMatrixBotMentionedDoesNotMatchSubstring(t *testing.T) {
	content := map[string]any{
		"body": "@sophia-helper:example.com ping",
	}
	if isMatrixBotMentioned("@sophia:example.com", content) {
...
func TestIsMatrixBotMentionedDoesNotMatchPlainMatrixURL(t *testing.T) {
	content := map[string]any{
		"body": "see https://matrix.to/#/@sophia:example.com",
	}
	if isMatrixBotMentioned("@sophia:example.com", content) {
...
func TestValidateConnectionRejectsTokenUserMismatch(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "token belongs to @alice:example.com, expected @sophia:example.com") {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(requests) != 2 {
...
func TestValidateConnectionSkipsSyncProbe(t *testing.T) {
	...
	adapter.httpClient = &http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		case "/_matrix/client/v3/account/whoami":
			return &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(`{"user_id":"@sophia:example.com"}`)),
				Header:     make(http.Header),
			}, nil
		case "/_matrix/client/v3/sync":
...
func TestStripMatrixReplyFallback(t *testing.T) {
	body := "> <@sophia:example.com> This looks like Antelope Canyon\n>\nWhere is Antelope Canyon?"
...
func TestMatrixHandleEventExpandsRepliedImageContext(t *testing.T) {
	...
	adapter.httpClient = &http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		return &http.Response{
			...
			Body: io.NopCloser(strings.NewReader(`{
				"event_id":"$img1",
				"type":"m.room.message",
				"sender":"@sophia:example.com",
				"unsigned":{"displayname":"Sophia"},
				"content":{
					"msgtype":"m.image",
					"body":"canyon.jpg",
	...
	delivered, err := adapter.handleEvent(
		...
		Config{HomeserverURL: "https://matrix.example.com", AccessToken: "tok", UserID: "@sophia:example.com"},
		matrixEvent{
			EventID: "$evt2",
			Type:    "m.room.message",
	...
	if captured.Message.Reply.Sender != "Sophia" {
...
func TestMatrixHandleEventUsesImageCaptionAsMessageText(t *testing.T) {
	...
	if alias, _ := captured.Metadata["bot_alias"].(string); alias != "@sophia:example.com" {
			...
			return &http.Response{
				...
				Body: io.NopCloser(strings.NewReader(`{
					"joined": {
						...
						"@sophia:example.com": {"display_name": "Sophia"}
					}
				}`)),
				Header: make(http.Header),
			}, nil
...
func TestMatrixSyncOnceAutoJoinsInvitedRooms(t *testing.T) {
	...
	nextSince, healthy, err := adapter.syncOnce(
		context.Background(),
		channel.ChannelConfig{ID: "cfg-1", BotID: "bot-1"},
		Config{HomeserverURL: "https://matrix.example.com", AccessToken: "tok", UserID: "@sophia:example.com", SyncTimeoutSeconds: 30, AutoJoinInvites: true},
		"s123",
		func(_ context.Context, _ channel.ChannelConfig, msg channel.InboundMessage) error {
			captured = msg
			return nil
...
func TestEnsureDirectRoomReusesExistingRoom(t *testing.T) {
	...
	adapter.httpClient = &http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		case "/_matrix/client/v3/rooms/!dm:example.com/joined_members":
			...
			return &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(`{"joined":{"@sophia:example.com":{},"@alice:example.com":{}}}`)),
				Header:     make(http.Header),
			}, nil
