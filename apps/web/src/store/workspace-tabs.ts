...
function syncTerminalGroupChrome(group: DockviewGroupPanel) {
  ...
  group.element.classList.toggle('sophia-terminal-group', terminalOnly)
...
export const useWorkspaceTabsStore = defineStore('workspace-tabs', () => {
  ...
  // mirrors this onto the dock root as `.sophia-dock-dragging`, and the theme
  ...
  function registerApi(dock: DockviewApi) {
    ...
    apiDisposables = [
      ...
      domListener('dragover', (event) => {
        ...
        if (!target.closest('.sophia-terminal-group')) return
