...
func TestExecuteResultUsesProvidedInvocation(t *testing.T) {
	t.Parallel()
	h := newTestHandler(&fakeRoleResolver{role: "owner"})
	invocation, err := ParseInvocation(InvocationInput{Text: "/help @sophia1bot", BotAliases: []string{"sophia1bot"}})
	if err != nil {
...
func TestExecute_TelegramBotSuffix(t *testing.T) {
	t.Parallel()
	h := newTestHandler(&fakeRoleResolver{role: "owner"})
	result, err := h.Execute(context.Background(), "bot-1", "user-1", "/help@SophiaBot")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !strings.Contains(result, "Available commands") {
