...
func TestACPRuntimeHandlerEnsureStartsRuntimeAndReturnsModels(t *testing.T) {
	t.Setenv("SOPHIA_ACP_MCP_HTTP_BASE_URL", "http://example.com")
...
func TestBuildACPMCPToolsURLUsesOnlyExplicitOrLoopbackBaseURL(t *testing.T) {
	...
	t.Run("explicit base URL", func(t *testing.T) {
		t.Setenv("SOPHIA_ACP_MCP_HTTP_BASE_URL", "https://sophia.example")
		...
		want := "https://sophia.example/bots/" + botID + "/tools"
		if got != want {
	...
	t.Run("non-loopback request host", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodPost, "https://sophia.example/acp-runtime", nil)
		req.Header.Set("X-Forwarded-Host", "evil.example")
