...
SELECT COALESCE((
  ...
  WHERE r.team_id = public.sophia_current_team_id() AND r.bot_id = b.id
    AND r.enabled = true
...
FROM bots b
WHERE b.team_id = public.sophia_current_team_id() AND b.id = sqlc.arg(bot_id);

-- name: GetBotACLDefaultEffect :one
SELECT acl_default_effect FROM bots WHERE team_id = public.sophia_current_team_id() AND id = $1;

-- name: SetBotACLDefaultEffect :exec
UPDATE bots SET acl_default_effect = $2, updated_at = now() WHERE team_id = public.sophia_current_team_id() AND id = $1;
...
FROM bot_acl_rules r
LEFT JOIN channel_identities ci ON ci.id = r.channel_identity_id AND ci.team_id = public.sophia_current_team_id()
LEFT JOIN bot_channel_routes source_route ON source_route.team_id = public.sophia_current_team_id()
...
WHERE r.team_id = public.sophia_current_team_id() AND r.bot_id = $1
...
DELETE FROM bot_acl_rules WHERE team_id = public.sophia_current_team_id() AND id = $1;
