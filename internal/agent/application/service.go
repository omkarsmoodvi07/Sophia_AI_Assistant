...
func (s *Service) prepareRunConfig(ctx context.Context, cfg native.RunConfig) native.RunConfig {
	...
	// Sophia's persona goes on last so that where it and the platform's default
	// tone instructions disagree about how to sound, hers is the more recent
	// instruction. Everything the platform says about tools and safety is earlier
	// in the prompt and untouched. See service_persona.go for why this is here in
	// the system prompt rather than in the memory context message where it used
	// to be, and why it must not be conditional on anything.
	if persona := personaBlock(cfg.Bot); persona != "" {
		cfg.System += "\n\n" + persona
	}
