...
func TestCodexACPLiveContainerAPIKey(t *testing.T) {
	if os.Getenv("SOPHIA_LIVE_CODEX_ACP_CONTAINER") != "1" {
		t.Skip("set SOPHIA_LIVE_CODEX_ACP_CONTAINER=1 to run the live Codex ACP container api_key smoke test")
	}
	...
	if err := os.WriteFile(filepath.Join(project, "README.md"), []byte("# Sophia ACP container live smoke\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	result, err := runner.Run(ctx, RunRequest{
		...
		Task:        "Reply with exactly this text and do not modify files: sophia-acp-container-live-ok",
	...
	if !strings.Contains(strings.ToLower(result.Text), "sophia-acp-container-live-ok") {
		t.Fatalf("live container api_key Codex ACP text = %q, want marker sophia-acp-container-live-ok", result.Text)
	}
}

func TestACPContainerAPIKeyCredentialNotInPSEF(t *testing.T) {
	if os.Getenv("SOPHIA_LIVE_CODEX_ACP_CONTAINER") != "1" {
		t.Skip("set SOPHIA_LIVE_CODEX_ACP_CONTAINER=1 to run the live ACP container ps smoke test")
	}
	if _, err := exec.LookPath("docker"); err != nil {
	...
	const secret = "sk-sophia-ps-secret" //nolint:gosec // live test uses a fake secret to verify process listing redaction.
...
func startLiveBridgeContainer(t *testing.T, dataRoot string) *bridge.Client {
	...
	image := strings.TrimSpace(os.Getenv("SOPHIA_LIVE_CODEX_ACP_CONTAINER_IMAGE"))
	if image == "" {
		image = "sophia-toolkit-acp-bridge-live:local"
