...
WITH target AS (
  ...
  WHERE request.team_id = public.sophia_current_team_id() AND request.session_id = $2
    AND request.id = $3
...
FROM next_seq
WHERE m.team_id = public.sophia_current_team_id() AND m.id = $1
...
WITH latest AS (
  ...
  WHERE visible.team_id = public.sophia_current_team_id() AND visible.session_id = $2
...
WITH target AS (
  ...
  FROM bot_history_messages request
  WHERE request.team_id = public.sophia_current_team_id() AND request.session_id = $1
    AND request.id = $2
    AND request.turn_id IS NOT NULL
    AND request.turn_visible = true
    AND NOT EXISTS (
...
linked AS (
  ...
  SET turn_id = target.turn_id,
      ...
      turn_superseded_reason = NULL
  FROM target
  WHERE assistant.team_id = public.sophia_current_team_id() AND assistant.id = $3
...
linked AS (
  ...
  SET turn_id = target.turn_id,
      ...
      turn_superseded_reason = NULL
  FROM target
  WHERE assistant.team_id = public.sophia_current_team_id() AND assistant.id = $2
    AND assistant.session_id = target.session_id
  RETURNING assistant.id AS assistant_message_id, assistant.created_at AS assistant_created_at
)
SELECT target.turn_id AS id, target.bot_id, target.session_id, target.turn_position::bigint AS position,
...
WITH next_position AS (
  UPDATE bot_sessions
  SET next_turn_position = next_turn_position + 1
  WHERE bot_sessions.team_id = public.sophia_current_team_id() AND bot_sessions.id = $1
...
WITH target AS (
  ...
  FROM bot_history_messages existing
  WHERE existing.team_id = public.sophia_current_team_id() AND existing.turn_id = $17
...
WITH target AS (
  ...
  WHERE m.team_id = public.sophia_current_team_id() AND m.turn_id = $1
    AND m.session_id = $2
...
WITH target AS (
  SELECT m.turn_id, m.session_id, m.turn_position
  FROM bot_history_messages m
  WHERE m.team_id = public.sophia_current_team_id() AND m.session_id = $1
...
WITH target AS (
  SELECT
    ...
    bool_or(existing.turn_visible) AS turn_visible
  FROM bot_history_messages existing
  WHERE existing.team_id = public.sophia_current_team_id() AND existing.turn_id = $3
    AND existing.turn_position IS NOT NULL
  GROUP BY existing.turn_id, existing.session_id, existing.turn_position
  LIMIT 1
)
...
SET turn_id = target.turn_id,
    ...
    turn_superseded_reason = NULL
FROM target
WHERE m.team_id = public.sophia_current_team_id() AND m.id = $2
  AND m.session_id = target.session_id
RETURNING m.id
`
...
WITH target AS (
  ...
  FROM bot_history_messages assistant
  WHERE assistant.team_id = public.sophia_current_team_id() AND assistant.turn_id = $1
    AND assistant.role = 'assistant'
    AND assistant.turn_message_seq = 2
    AND assistant.turn_visible = true
  LIMIT 1
...
WHERE m.team_id = public.sophia_current_team_id() AND m.session_id = $1
  ...
  AND (m.turn_position, m.turn_message_seq, m.created_at, m.id)
    > (
      ...
      WHERE c.team_id = public.sophia_current_team_id() AND c.session_id = $1
...
replacement_input AS (
  ...
  WHERE (
      ...
      OR EXISTS (
        ...
        WHERE request_message.team_id = public.sophia_current_team_id() AND request_message.id = $3::uuid
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE assistant_message.team_id = public.sophia_current_team_id() AND assistant_message.id = $4::uuid
...
updated AS (
  ...
  SET turn_superseded_by_turn_id = replacement.id,
      ...
      turn_visible = false
  FROM replacement
  WHERE old.team_id = public.sophia_current_team_id() AND old.turn_id = $2
