...
| Data Fetching | Pinia Colada (`@pinia/colada`) + `@sophiaai/sdk` |
| Forms | vee-validate + `@vee-validate/zod` + Zod |
| i18n | vue-i18n (en / zh) |
| Icons | lucide-vue-next (primary) + `@sophiaai/icon` (brand/provider icons) |
...
- `/dev/*` is registered only in dev builds and requires `localStorage.setItem('sophia:dev-tools', '1')`.
...
`pages/dev/components/` is the living reference for `@felinic/ui` atoms and tokens. Reach it at `/dev/components` in dev builds after `localStorage.setItem('sophia:dev-tools', '1')`. Also openable via `Cmd/Ctrl+Shift+D` when dev tools are enabled. Use it to verify visual changes; `mise run lint` runs `scripts/check-ui-contract.mjs` as a mechanical guard.
...
- **`@sophiaai/icon`** (brand icons): Workspace package (`packages/icons/`) providing AI provider, search engine, and channel platform SVG icons as Vue components. Example: `import { Openai, Claude } from '@sophiaai/icon'`.
- **Do NOT use FontAwesome** for new code. Legacy FontAwesome usage remains only in commented-out code blocks. Always use Lucide for UI icons and `@sophiaai/icon` for brand logos.
...
- SDK: `@sophiaai/sdk` auto-generated from OpenAPI via `@hey-api/openapi-ts`
...
- **Parsing**: handled by the generated SDK (`@sophiaai/sdk` `sse.get`); wrappers live in `composables/api/useChat.message-api.ts`.
...
- Aliases: `@` → `./src`, `#` → `../ui/src`
- Config: reads from `SOPHIA_CONFIG_PATH` / `CONFIG_PATH` when provided, otherwise `../../config.toml`, via `@sophiaai/config`
...
- Use `lucide-vue-next` for all UI icons. Use `@sophiaai/icon` for brand/provider logos. **Never use FontAwesome** — do not add `<FontAwesomeIcon>`, do not import from `@fortawesome/*`, do not use inline SVG or base64-encoded SVG in templates.
- Use Pinia Colada (`useQuery`/`useMutation`) for server state; use Pinia stores for client state only.
- API calls must go through `@sophiaai/sdk`; never call `fetch()` directly.
