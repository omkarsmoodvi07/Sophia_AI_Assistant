...
// bridge TCP 通道的 strict mTLS（设计 sophia-saas-bridge-mtls-design.md §8.1）。
// 材料由 sophia-bridge-mtls Secret 挂载，env 只传路径与模式，私钥不进 env。
const (
...
func bridgeServerCredentials() (credentials.TransportCredentials, error) {
	...
	cfg := &tls.Config{
		...
		// 钉死到本 instance 的 Sophia Server SPIFFE 身份。被攻破的 bot 只持有
		// shared bridge server cert（ServerAuth、bridge URI），过不了这一关。
		VerifyConnection: func(cs tls.ConnectionState) error {
			return verifySophiaServerClientIdentity(cs, expectedURI)
		},
	}
	return credentials.NewTLS(cfg), nil
}

func verifySophiaServerClientIdentity(cs tls.ConnectionState, expectedURI string) error {
