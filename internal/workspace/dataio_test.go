...
func TestTarGzDirSkipsHermesSecrets(t *testing.T) {
	...
	writeTestSymlink(t, root, "../.sophia-hermes/.env", "notes/env-link")
...
func TestImportDataViaGRPCSkipsHermesSecrets(t *testing.T) {
	...
	writeTestFile(t, root, ".sophia-hermes/mcp-tokens/stale-target.json", `{"refresh_token":"old"}`)
	...
	assertPathMissing(t, root, ".sophia-hermes/mcp-tokens/stale-target.json")
...
func workspaceArchiveFixture() map[string]string {
	...
	files[".sophia-hermes/config.yaml"] = "model:\n  provider: openrouter\n"
...
func workspaceSecretFiles() map[string]string {
	return map[string]string{
		".sophia-hermes/.env":                       "OPENROUTER_API_KEY=secret\n",
		".sophia-hermes/auth.json":                  `{"token":"secret"}`,
		".sophia-hermes/auth/google_oauth.json":     `{"refresh_token":"secret"}`,
		".sophia-hermes/mcp-tokens/github.json":     `{"refresh_token":"secret"}`,
		".sophia-hermes/sessions/latest.json":       `{"token":"secret"}`,
		".sophia-hermes/state.db":                   "secret",
		".sophia-hermes/state.db-wal":               "secret",
		".sophia-hermes/accounts/default/.env":      "OPENAI_API_KEY=secret\n",
		".sophia-hermes/accounts/default/auth.json": `{"token":"secret"}`,
...
func assertWorkspaceUserDataInArchive(t *testing.T, names []string) {
	t.Helper()
	for _, path := range []string{".sophia-hermes/config.yaml", ".hermes/config.yaml", ".codex/config.toml", "notes/readme.txt"} {
		if !hasArchiveName(names, path) {
...
func assertWorkspaceUserDataOnDisk(t *testing.T, root string) {
	t.Helper()
	assertPathContent(t, root, ".sophia-hermes/config.yaml", "model:\n  provider: openrouter\n")
