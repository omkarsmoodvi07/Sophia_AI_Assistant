...
func TestDisplayPrepareCommandUsesImageScripts(t *testing.T) {
	...
	if got := displayPrepareCommand(); got != "/bin/sh /opt/sophia/scripts/display-prepare.sh" {
	...
	if strings.Contains(prepareScript, "cat >/tmp/sophia-") {
	...
	if !strings.Contains(cmd, "sophia-logo-white") || strings.Contains(cmd, "sophia-apple-symbol") {
		t.Fatal("injected style script must use the white Sophia logo for the topbar menu icon")
	}
	if !strings.Contains(cmd, "sophia_logo_png_base64()") ||
		!strings.Contains(cmd, "sophia-logo-white.png") ||
		!strings.Contains(cmd, "/usr/share/icons/hicolor/48x48/apps") ||
		!strings.Contains(cmd, "base64 -d") {
		t.Fatal("injected style script must install a PNG Sophia logo fallback for panels without SVG icon loading")
	}
	...
	if filesIndex < 0 || filesLauncherIndex < filesIndex || thunarFallbackIndex < filesLauncherIndex ||
		!strings.Contains(cmd, "Sophia-WhiteSur-dark") ||
		...
		!strings.Contains(cmd, `require_xfconf_value xsettings /Net/IconThemeName Sophia-WhiteSur-dark`) ||
		!strings.Contains(cmd, "restart_file_manager()") ||
		strings.Contains(cmd, "sophia_files_icon_png_base64") ||
		strings.Contains(cmd, "write_sophia_files_icon_png") {
	...
	if !strings.Contains(prepareScript, "/opt/sophia/scripts/display-apply-style.sh --check") ||
		!strings.Contains(prepareScript, "/opt/sophia/scripts/display-apply-style.sh --ensure") {
		t.Fatal("display prepare must call the image-provided style helper")
	}
	if !strings.Contains(prepareScript, `desktop_style_current()`) ||
		!strings.Contains(prepareScript, `/bin/sh /opt/sophia/scripts/display-apply-style.sh --check`) {
		t.Fatal("display prepare readiness must include the desktop style marker")
	}
	if strings.Contains(prepareScript, `nohup /bin/sh /opt/sophia/scripts/desktop-style.sh`) {
	...
	if !strings.Contains(prepareScript, `/bin/sh /opt/sophia/scripts/display-apply-style.sh --ensure`) {
	...
	if !strings.Contains(cmd, "SOPHIA_DISPLAY_INSTALL_ASSETS_GLOBAL") ||
	...
	if !strings.Contains(prepareScript, `XVNC_GEOMETRY="${SOPHIA_DISPLAY_GEOMETRY:-1280x960}"`) {
...
func TestDisplayApplyStyleCommandUsesImageScripts(t *testing.T) {
	...
	if got := displayApplyStyleCommand(); got != "/bin/sh /opt/sophia/scripts/display-apply-style.sh --if-needed" {
		t.Fatalf("displayApplyStyleCommand() = %q", got)
	}
	applyScript := readDisplayScript(t, "display-apply-style.sh")
	styleScript := readDisplayScript(t, "desktop-style.sh")
	cmd := applyScript + "\n" + styleScript
	if strings.Contains(cmd, "cat >/tmp/sophia-") || strings.Contains(cmd, "desktop-install.sh") {
		t.Fatal("display style apply must not inject scripts or install packages at runtime")
	}
	if !strings.Contains(applyScript, `desktop_style_script="${SOPHIA_DESKTOP_STYLE_SCRIPT:-/opt/sophia/scripts/desktop-style.sh}"`) ||
	...
	if !strings.Contains(cmd, "style_lock=/tmp/sophia-desktop-style.lock") ||
	...
	if !strings.Contains(cmd, "sophia-logo-white") || strings.Contains(cmd, "sophia-apple-symbol") {
		t.Fatal("display style command must use the white Sophia logo for the topbar menu icon")
	}
	if !strings.Contains(cmd, "sophia_logo_png_base64()") ||
		!strings.Contains(cmd, "sophia-logo-white.png") ||
		!strings.Contains(cmd, "/usr/share/icons/hicolor/48x48/apps") ||
		!strings.Contains(cmd, "base64 -d") {
		t.Fatal("display style command must install a PNG Sophia logo fallback for panels without SVG icon loading")
	}
	if !strings.Contains(cmd, "xfconf_replace_int_array xfce4-panel /panels 1") || !strings.Contains(cmd, "restart_xfce_panel()") {
...
func TestDisplayStyleStatusCommandChecksVersionMarker(t *testing.T) {
	...
	if got := displayStyleStatusCommand(); got != "/bin/sh /opt/sophia/scripts/display-apply-style.sh --check" {
	...
	if !strings.Contains(cmd, "SOPHIA_DISPLAY_DESKTOP_STYLE") {
...
func TestWorkspaceDockerfileInstallsDisplayAssetsGlobally(t *testing.T) {
	...
	if !strings.Contains(dockerfile, "COPY scripts/desktop-install.sh /tmp/sophia-desktop-install.sh") {
		t.Fatal("workspace Dockerfile must install display assets through the shared install script")
	}
	if !strings.Contains(dockerfile, "SOPHIA_DISPLAY_INSTALL_ASSETS_GLOBAL=1") {
		t.Fatal("workspace Dockerfile must bake static display style assets into global image paths")
	}
	if !strings.Contains(dockerfile, "SOPHIA_TOOLKIT_GLIBC_ONLY=1") ||
		!strings.Contains(dockerfile, "COPY --from=toolkit-builder /opt/sophia/toolkit /opt/sophia/toolkit") {
		t.Fatal("workspace Dockerfile must own the canonical glibc toolkit")
	}
	if !strings.Contains(dockerfile, "COPY scripts/desktop-style.sh scripts/display-apply-style.sh scripts/display-prepare.sh /opt/sophia/scripts/") {
		t.Fatal("workspace Dockerfile must provide immutable display scripts")
	}
	if !strings.Contains(dockerfile, "COPY docker/workspace-contract.json /opt/sophia/workspace-contract.json") {
