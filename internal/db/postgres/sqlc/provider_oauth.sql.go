...
DELETE FROM provider_oauth_tokens WHERE team_id = public.sophia_current_team_id() AND provider_id = $1
`
...
SELECT id, provider_id, access_token, refresh_token, expires_at, scope, token_type, state, pkce_code_verifier, metadata, created_at, updated_at, team_id FROM provider_oauth_tokens WHERE team_id = public.sophia_current_team_id() AND provider_id = $1
`
...
SELECT id, provider_id, access_token, refresh_token, expires_at, scope, token_type, state, pkce_code_verifier, metadata, created_at, updated_at, team_id FROM provider_oauth_tokens WHERE team_id = public.sophia_current_team_id() AND state = $1 AND state != ''
`
