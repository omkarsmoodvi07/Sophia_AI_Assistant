...
import (
	...
	"github.com/sophiaai/sophia/internal/workspace/bridgepb"
...
func TestBrowserProxyURLAndHostParsing(t *testing.T) {
	...
	req.Header.Set("X-Forwarded-Host", "sophia.example.com")
	got = buildBrowserProxyURL(req, "0123456789abcdef0123456789abcdef", "/")
	want = "https://0123456789abcdef0123456789abcdef.browser.sophia.example.com/"
	if got != want {
		t.Fatalf("forwarded browser proxy url = %q, want %q", got, want)
	}

	id, ok := browserSessionIDFromHost("0123456789abcdef0123456789abcdef.browser.sophia.example.com:443")
	if !ok || id != "0123456789abcdef0123456789abcdef" {
		t.Fatalf("browser session id = %q, %v", id, ok)
	}
	if _, ok := browserSessionIDFromHost("not-a-session.browser.sophia.example.com"); ok {
...
func TestBrowserReverseProxyHTTPThroughBridgeTunnel(t *testing.T) {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/hello" || r.URL.RawQuery != "name=sophia" {
	...
	req := httptest.NewRequest(http.MethodPost, "http://session.browser.localhost/hello?name=sophia", bytes.NewBufferString("payload"))
