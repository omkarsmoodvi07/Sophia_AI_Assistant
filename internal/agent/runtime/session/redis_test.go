...
func uniqueRuntimeBackendPrefix(scope string) string {
	return fmt.Sprintf("sophia:test:session_runtime:%s:%d:%d:", scope, time.Now().UnixNano(), runtimeBackendPrefixSequence.Add(1))
}
...
func TestRedisValkeyRuntimeManagerContractOptional(t *testing.T) {
	targets := []struct {
		...
		{name: "redis", url: os.Getenv("SOPHIA_TEST_REDIS_URL")},
		{name: "valkey", url: os.Getenv("SOPHIA_TEST_VALKEY_URL")},
	}
	var ran bool
	...
	if !ran {
		if os.Getenv("SOPHIA_TEST_DISTRIBUTED_REQUIRED") == "1" {
			t.Fatal("distributed session runtime contracts are required, but neither SOPHIA_TEST_REDIS_URL nor SOPHIA_TEST_VALKEY_URL is set")
		}
		t.Skip("set SOPHIA_TEST_REDIS_URL and/or SOPHIA_TEST_VALKEY_URL to run Redis and/or Valkey session runtime contracts")
	}
}
...
func runRedisSubscriptionReconnectContract(t *testing.T, redisURL string) {
	...
	clientName := fmt.Sprintf("sophia-runtime-reconnect-%d", runtimeBackendPrefixSequence.Add(1))
