...
SELECT * FROM containers WHERE team_id = public.sophia_current_team_id() AND bot_id = sqlc.arg(bot_id) ORDER BY updated_at DESC LIMIT 1;

-- name: DeleteContainerByBotID :exec
DELETE FROM containers WHERE team_id = public.sophia_current_team_id() AND bot_id = sqlc.arg(bot_id);
...
SELECT * FROM containers WHERE team_id = public.sophia_current_team_id() AND auto_start = true ORDER BY updated_at DESC;
