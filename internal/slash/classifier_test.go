...
func TestClassifyChannelDirectSkillActivationWithBotSuffix(t *testing.T) {
	decision := Classify(ClassifyInput{
		Text:       "/alpha@Sophia do it",
		Surface:    SurfaceChannel,
		IsGroup:    true,
		BotAliases: []string{"Sophia"},
	})
	if decision.Kind != DecisionSkillIntent {
		t.Fatalf("kind = %s, want %s", decision.Kind, DecisionSkillIntent)
	}
...
func TestClassifyChannelCommandAddressingForms(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "leading mention", text: "@sophia1bot /new discuss", want: "/new discuss"},
		{name: "command suffix", text: "/new@sophia1bot discuss", want: "/new discuss"},
		{name: "action suffix", text: "/new discuss@sophia1bot", want: "/new discuss"},
		{name: "mention after command", text: "/new @sophia1bot", want: "/new"},
		{name: "mention after action", text: "/new discuss @sophia1bot", want: "/new discuss"},
		{name: "multiple leading mentions", text: "@alice @sophia1bot /new discuss", want: "/new discuss"},
	}
