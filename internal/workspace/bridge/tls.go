...
// TLSOptions 配置 Sophia Server → bridge 的 TCP gRPC mTLS（设计 §8.2）。
// nil 表示 disabled（开源/本地默认）。strict 下握手或身份校验失败一律拒绝，
// 不回退明文；UDS target 不论 mode 始终走本地信任模型（insecure）。
type TLSOptions struct {
	// ServerName 是 synthetic TLS ServerName（sophia-bridge.<instance-id>.bridge.sophia.internal）。
	...
	// （spiffe://sophia/instance/<instance-id>/bridge）。
