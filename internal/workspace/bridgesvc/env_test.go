...
func TestExecEnvUnsetsInheritedEnvBeforeAppendingOverrides(t *testing.T) {
	...
	env := execEnv(&pb.ExecInput{
		Env:      []string{"HERMES_HOME=/data/.sophia-hermes", "PATH=/toolkit"},
	...
	if !hasEnvValue(env, "HERMES_HOME", "/data/.sophia-hermes") {
...
func TestExecPTYEnvInheritsDefaultEnvironment(t *testing.T) {
	t.Setenv("SOPHIA_TEST_PTY_SENTINEL", "present")
	env := execPTYEnv(&pb.ExecInput{})
	if !hasEnvValue(env, "SOPHIA_TEST_PTY_SENTINEL", "present") {
