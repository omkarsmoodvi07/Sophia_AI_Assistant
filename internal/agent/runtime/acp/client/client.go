...
func (c *clientCallbacks) RequestPermission(ctx context.Context, p acp.RequestPermissionRequest) (acp.RequestPermissionResponse, error) {
	...
	if !native {
		...
		// Only protocol-level ACP permissions and confirmed Sophia native MCP
		// tool preflights are allowed here. Unknown shapes fail closed above so
		// a new write/exec encoding cannot bypass Sophia's approval policy.
		if c.logger != nil {
			title, kind := permissionToolIdentity(p)
...
func (c *clientCallbacks) allowUnmappedPermission(ctx context.Context, p acp.RequestPermissionRequest) bool {
	...
	case string(acp.ToolKindOther), "":
		...
		return c.allowsSophiaMCPToolPreflight(ctx, preflight)
	default:
		return false
	}
}
...
func (c *clientCallbacks) allowsSophiaMCPToolPreflight(ctx context.Context, preflight mcpPermissionPreflight) bool {
	...
	if !isSophiaToolsMCPServerName(preflight.serverName) {
		if c.logger != nil {
			c.logger.Warn("cancelling MCP tool preflight for missing or non-Sophia server",
	...
	// tool name. The actual call still goes through Sophia's scoped tool
	// gateway; when the name is present, classify it against that same gateway
	// so ACP sees the native-aligned Sophia tool surface.
