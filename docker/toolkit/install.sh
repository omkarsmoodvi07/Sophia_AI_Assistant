...
OUTDIR="$1"
ARCH="${2:-}"
DISPLAY_OUTDIR="${SOPHIA_DISPLAY_OUTDIR:-${3:-$OUTDIR/display}}"
...
install_python_runtime() {
  ...
  marker="$dest_dir/.sophia-python-version"
...
install_a11y_cli() {
  ...
  if [ -n "${SOPHIA_A11Y_CLI_BINARY:-}" ] && is_linux_elf "$SOPHIA_A11Y_CLI_BINARY"; then
    cp "$SOPHIA_A11Y_CLI_BINARY" "$dest_path"
    chmod +x "$dest_path"
    echo "a11y-cli installed from $SOPHIA_A11Y_CLI_BINARY"
    return
  fi
  ...
  echo "         Run 'mise run a11y-cli:build' or set SOPHIA_A11Y_CLI_BINARY to a Linux ELF." >&2
}
...
if [ "${SOPHIA_TOOLKIT_GLIBC_ONLY:-0}" != "1" ]; then
...
if [ "${SOPHIA_TOOLKIT_SKIP_DISPLAY:-0}" != "1" ]; then
  install_display_bundle
  install_a11y_cli
fi
