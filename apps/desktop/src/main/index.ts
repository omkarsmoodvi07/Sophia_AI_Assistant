...
const DESKTOP_PRODUCT_NAME = 'Sophia'
...
function getDesktopApiBaseUrl(): string {
  return resolveDesktopBaseUrl({
    ...
    desktop: process.env.SOPHIA_DESKTOP_BASE_URL,
    proxy: process.env.SOPHIA_WEB_PROXY_TARGET,
...
function getDesktopServerStatus() {
  ...
  return {
    baseUrl,
    managed: Boolean(
      process.env.SOPHIA_DESKTOP_BASE_URL?.trim()
      || process.env.SOPHIA_WEB_PROXY_TARGET?.trim()
...
function openSophiaSettings(): void {
...
function createChatWindow(): BrowserWindow {
  const window = new BrowserWindow({
    ...
    ...macWindowChromeOptions('sophia-chat'),
    show: false,
  ...
  if (process.platform === 'darwin') {
    template.push({
      ...
      submenu: [
        ...
        { type: 'separator' },
        {
          label: 'Sophia Settings...',
          click: openSophiaSettings,
        },
        { type: 'separator' },
        { role: 'services' },
        { type: 'separator' },
...
app.whenReady().then(async () => {
  ...
  electronApp.setAppUserModelId(process.env.SOPHIA_DESKTOP_APP_ID?.trim() || 'ai.sophia.desktop')
