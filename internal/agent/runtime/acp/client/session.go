...
func (r *Runner) StartSession(ctx context.Context, req StartRequest, sink EventSink) (*Session, error) {
	...
	if backend == WorkspaceBackendContainer && toolHTTPHandler != nil &&
		...
		if err != nil {
			cancel()
			return nil, fmt.Errorf("prepare Sophia tools bridge: %w", err)
		}
		toolHTTPURL = guardedURL
		var stop func()
		client, stop, err = r.startSophiaToolsBridge(lifecycleCtx, req.BotID, client, guardedPath, guardedHandler)
		if err != nil {
			if r.logger != nil {
				r.logger.Warn("Sophia tools bridge unavailable; starting ACP session without Sophia tools",
	...
	initResp, err := conn.Initialize(ctx, acp.InitializeRequest{
		...
		ClientInfo:      &acp.Implementation{Name: "sophia", Version: "dev"},
	...
	if initResp.AgentCapabilities.McpCapabilities.Http || forceHTTPMCPServer {
		if server := sophiaToolsHTTPMCPServer(toolHTTPURL, toolSession); server.Http != nil {
	...
	if r.logger != nil {
		...
		r.logger.Info("ACP agent initialized",
			...
			slog.Bool("sophia_tools_http_configured", toolHTTPURL != ""),
			slog.String("sophia_tools_http_url", redactedToolHTTPURL(toolHTTPURL)),
			slog.Int("mcp_servers", len(mcpServers)),
		)
		if toolHTTPURL != "" && len(mcpServers) == 0 {
			r.logger.Warn("Sophia tools were not exposed to ACP agent because no supported MCP transport was available",
				slog.String("agent_id", req.AgentID),
				slog.String("bot_id", req.BotID),
	...
	if defaultReasoning := strings.TrimSpace(req.DefaultReasoningEffort); defaultReasoning != "" && clientSession.ReasoningState().Supported {
		if _, err := clientSession.SetReasoningEffort(ctx, defaultReasoning); err != nil {
			if errors.Is(err, ErrReasoningEffortUnavailable) ||
				...
				// authoritative config snapshot never reached Sophia. Do not return a
...
func (r *Runner) startSophiaToolsBridge(ctx context.Context, botID string, client *bridge.Client, route string, handler http.Handler) (*bridge.Client, func(), error) {
	if client == nil {
...
func guardedToolHTTPURL(rawURL string) (string, string, error) {
	...
	if u.Scheme == "" || u.Host == "" {
		return "", "", fmt.Errorf("invalid Sophia tools URL %q", rawURL)
	}
