...
describe('botCreateTerminal', () => {
  it('pushes a running pulling line carrying the image', () => {
    const lines = appendBotCreateTerminalLine([], { type: 'pulling', image: 'ghcr.io/sophia:latest' })

    expect(lines).toHaveLength(1)
    expect(lines[0]).toMatchObject({ kind: 'pulling', status: 'running', image: 'ghcr.io/sophia:latest' })
  })
