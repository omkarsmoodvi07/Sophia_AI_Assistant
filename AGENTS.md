...
The native desktop client is a separate distribution boundary for Sophia Cloud or a hosted Sophia server. `apps/desktop` reuses `@sophiaai/web` modules, but owns the Electron shell, system tray behavior, menus, preload IPC, cache invalidation, and packaged application resources.
...
- **AI SDK**: [Twilight AI](https://github.com/sophiaai/twilight-ai) (Go LLM SDK — OpenAI, Anthropic, Google)
...
- **Icons**: lucide-vue-next + `@sophiaai/icon` (brand/provider icons)
- **i18n**: vue-i18n
- **Markdown**: markstream-vue + Shiki + Mermaid + KaTeX
- **Desktop**: Electron 34 + [electron-vite](https://electron-vite.github.io/) 4 native client, reusing `@sophiaai/web` modules while managing the desktop renderer, tray behavior, menus, and preload IPC
...
│   ├── desktop/                #   Native Electron app (@sophiaai/desktop): hosted-server renderer, tray, menus, preload IPC
│   └── web/                    #   Main web app (@sophiaai/web, Vue 3) — see apps/web/AGENTS.md
├── packages/                   # Shared TypeScript libraries
│   ├── ui/                     #   Shared UI component library (@felinic/ui) — git submodule → github.com/sophiaai/ui; its AGENTS.md routes agents to the UI-owned Web guidance
│   ├── sdk/                    #   TypeScript SDK (@sophiaai/sdk, auto-generated from OpenAPI)
│   ├── icons/                  #   Brand/provider icon library (@sophiaai/icon)
│   └── config/                 #   Shared configuration utilities (@sophiaai/config)
...
| `mise run desktop:dev` | Start Electron desktop app in dev mode (renderer reuses @sophiaai/web) |
...
Optional profile: `webhook-tunnel` (cloudflared for channels behind NAT). Desktop connects to Sophia Cloud or this hosted server instead of running its own server.
...
4. The frontend calls APIs via the auto-generated `@sophiaai/sdk`.
...
- API calls use the auto-generated `@sophiaai/sdk`.
...
- `apps/desktop/` is an [electron-vite](https://electron-vite.github.io/) project (`@sophiaai/desktop`) with its own managed renderer bootstrap for Sophia Cloud or a hosted Sophia server. It reuses exported `@sophiaai/web` pages, layouts, stores, i18n, API setup, and design tokens, but owns the Electron shell instead of importing the full web `main.ts`.
...
- Desktop connects to the target server through `SOPHIA_DESKTOP_BASE_URL` and must not start a server, package database files, embed Qdrant, or install a companion CLI.
- Packaging is handled by `electron-builder` (config in `apps/desktop/electron-builder.yml`); output lands in `apps/desktop/dist/`.
- When desktop needs to diverge from the web experience, extend the desktop bootstrap or add explicit `@sophiaai/web` subpath exports plus desktop type stubs. Do **not** fork `apps/web` itself.
...
- The bridge binary (`cmd/bridge/`) runs inside each container as a read-only file mount, with UDS sockets under `/run/sophia/`. Toolkit binaries, display dependencies, and runtime scripts come from the versioned workspace image contract. When display is enabled the bridge can supervise Xvnc and a headed Chrome/Chromium process with CDP on port `9222`; the web UI then exposes a Display pane backed by screenshots/WebRTC/input forwarding. Treat VNC as the container desktop transport, not as the whole browser automation feature.
- The canonical workspace image is built from `docker/Dockerfile.workspace`. Compatible custom/provider images must expose the same `/opt/sophia/workspace-contract.json`, toolkit, and script paths.
