...
const computerDisplayReadyCommand = `# sophia-computer-display-ready-probe
...
func (p *BrowserProvider) ensureComputerDisplay(ctx context.Context, botID string) (*bridge.Client, error) {
	...
	result, err := client.Exec(ctx, "/bin/sh /opt/sophia/scripts/display-prepare.sh", "/", computerDisplayStartupTimeout)
	if err != nil {
...
nohup "$BROWSER" \
  ...
  --user-data-dir=/tmp/sophia-display-browser \
  about:blank >/tmp/sophia-browser.log 2>&1 &
`
...
func (p *cdpPage) removeAnnotations(ctx context.Context) error {
	_, err := p.evaluate(ctx, `(() => { document.querySelectorAll('.__sophia_annotation__').forEach(el => el.remove()); return true })()`)
	return err
}

func (p *cdpPage) accessibilitySnapshot(ctx context.Context) (string, error) {
	value, err := p.evaluate(ctx, `(() => sophiaInteractiveElements().slice(0, 300).map(({ ref, role, name, tag, selector }) => ({ ref, role, name, tag, selector })))()`)
	if err != nil {
		return "", err
	}
	raw, _ := json.Marshal(value)
...
const sophiaInteractiveSelector = [
  'a[href]',
  'button',
  'input',
  'select',
...
function sophiaCssEscape(value) {
  if (globalThis.CSS && typeof CSS.escape === 'function') return CSS.escape(value);
  return String(value).replace(/[^a-zA-Z0-9_-]/g, '\\$&');
}

function sophiaCssPath(el) {
  if (el.id) return '#' + sophiaCssEscape(el.id);
  const parts = [];
  let node = el;
...
function sophiaInteractiveElements() {
  const result = [];
  const seen = new Set();
  for (const el of document.querySelectorAll(sophiaInteractiveSelector)) {
    if (seen.has(el)) continue;
    seen.add(el);
    const rect = sophiaVisible(el);
    if (!rect) continue;
    ...
    result.push({
      ...
      role: sophiaRole(el),
      name: sophiaElementName(el),
      selector: sophiaCssPath(el)
    });
  }
  return result;
}
...
function elementByRef(ref) {
  ...
  const item = sophiaInteractiveElements()[index - 1];
