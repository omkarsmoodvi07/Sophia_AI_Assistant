...
func BridgeTLSOptionsFromConfig(cfg config.Config) (*bridge.TLSOptions, error) {
	...
	if instanceID == "" {
		return nil, errors.New("bridge tls: strict mode requires instance id (SOPHIA_INSTANCE_ID)")
	}
	dir := strings.TrimSpace(cfg.BridgeTLS.ServerDir)
	if dir == "" {
		return nil, errors.New("bridge tls: strict mode requires server material dir (SOPHIA_BRIDGE_TLS_SERVER_DIR)")
	}
...
func BridgeTLSRuntimeOptionsFromConfig(cfg config.Config) (*BridgeTLSRuntimeOptions, error) {
	...
	if bridgeDir == "" {
		return nil, errors.New("bridge tls: strict mode requires bridge material dir (SOPHIA_BRIDGE_TLS_BRIDGE_DIR)")
	}
