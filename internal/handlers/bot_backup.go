...
func (h *BotBackupHandler) Export(c echo.Context) error {
	...
	tmp, err := os.CreateTemp("", "sophia-backup-*.zip")
	if err != nil {
	...
	filename := fmt.Sprintf("bot-%s-backup-%s.sophia.zip", safeFilename(bot.DisplayName, bot.ID), time.Now().UTC().Format("20060102T150405Z"))
