...
func TestBridgeTLSOptionsFromConfigStrict(t *testing.T) {
	...
	if opts.ServerName != "sophia-bridge."+cfg.InstanceID+".bridge.sophia.internal" {
		t.Fatalf("derived ServerName = %q", opts.ServerName)
	}
	if opts.ExpectedServerURI != "spiffe://sophia/instance/"+cfg.InstanceID+"/bridge" {
...
func TestBridgeTLSRuntimeOptionsFromConfigStrict(t *testing.T) {
	...
	if opts.ExpectedClientURI != "spiffe://sophia/instance/"+cfg.InstanceID+"/server" {
