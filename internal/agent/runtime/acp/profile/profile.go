...
type Profile struct {
	...
	// launcher whose version can be refreshed independently of Sophia. The
	...
	Command        string
	Args           []string
	// SessionModeID, when set, is the ACP session mode Sophia pins right after
...
func claudeCodeProfile() Profile {
	return Profile{
		...
		// acceptEdits) silently bypasses Sophia's approval flow.
